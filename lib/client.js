window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function buildPanel() {
      ...
      panel.innerHTML =
        ...
        '<div id="dsb-settings" class="dsb-settings" style="display:none">' +
          ...
          '<span class="dsb-set-hint">悬浮球可拖动；清单面板始终居中</span>' +
        '</div>' +
    ...
    function paintCards() {
      ...
      // ── 推荐插件广告卡（同款推荐位，点击直达 GitHub）──
      html += '<div class="dsb-card" data-ext-url="https://github.com/loyalchiiina/dsh-chat-image-lightbox" title="点击打开 GitHub 仓库" style="border-color:rgba(255,210,122,.35)">' +
        '<div class="dsb-card-name" style="color:#ffd27a">🖼️ 推荐 · 照片显示器</div>' +
        '<div class="dsb-card-zh">聊天图片灯箱：多图自动网格、全屏放大、高清下载——本插件悬浮球导航的好搭档</div>' +
        '<div class="dsb-card-tag">dsh-chat-image-lightbox · 点击访问 ⭐ Star</div></div>';
      html += '<div class="dsb-card" data-ext-url="https://github.com/loyalchiiina/dsh-font-enhancer" title="点击打开 GitHub 仓库" style="border-color:rgba(255,210,122,.35)">' +
        '<div class="dsb-card-name" style="color:#ffd27a">🔤 推荐 · 字体插件</div>' +
        '<div class="dsb-card-zh">界面分区自定义字体/颜色/字号，自带字体球与一键装字体——和技能球并排两个悬浮球</div>' +
        '<div class="dsb-card-tag">dsh-font-enhancer · 点击访问 ⭐ Star</div></div>';
      html += '<div class="dsb-card" style="cursor:default;opacity:.85">' +
        '<div class="dsb-card-name" style="color:#8fc7ff">💡 悬浮球显隐设置</div>' +
        '<div class="dsb-card-zh">装了带悬浮球的插件（🧩 技能球 / 🔤 字体球）后，可到 <b>DSH 设置页 → 「悬浮球导航」分区</b> 勾选控制每个悬浮球显示或隐藏；面板内 ⚙ 设置同样可开关。</div>' +
        '<div class="dsb-card-tag">DSH 设置 → 悬浮球导航</div></div>';
      box.innerHTML = html;
    }
    ...
    function SettingsSection(props) {
      ...
      // 推荐1：dsh-chat-image-lightbox（照片显示器）
    ...
    function migrateOldState() {
      try {
        ...
        if (ver !== "2") {
          ...
          localStorage.setItem("dsh-skill-browser-cfg-ver", "2");
        }
      } catch (e) {}
    }
